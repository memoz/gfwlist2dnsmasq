...
check_depends() {
    if ! which sed base64 mktemp >/dev/null; then
...
check_filepath() {
    FILE="$1"
    if [ -z ${FILE##*/} ]; then
        _red 'Error: '$FILE' is a path, not a file.\n'
        exit 1
    else
        if [ "${FILE}"a != "${FILE%/*}"a ] && [ ! -d "${FILE%/*}" ]; then
            _red 'Error: Folder do not exist: '"${FILE%/*}"'\n'
            exit 1
        fi
    fi
}
...
get_args() {
    ...
    if [ -n "$EXTRA_DOMAIN_FILE" ] && [ ! -f "$EXTRA_DOMAIN_FILE" ]; then
        _yellow 'WARNING:\nExtra domain file does not exist, ignored.\n\n'
        EXTRA_DOMAIN_FILE=''
    fi

    if [ -n "$EXCLUDE_DOMAIN_FILE" ] && [ ! -f "$EXCLUDE_DOMAIN_FILE" ]; then
...
process() {
    # Set Global Var
    BASE_URL='https://git.tuxfamily.org/gfwlist/gfwlist.git/plain/gfwlist.txt'
    ...
    if [ -n "$EXTRA_DOMAIN_FILE" ]; then
        printf 'Extra domain file %s... ' "$EXTRA_DOMAIN_FILE"
        cat "$EXTRA_DOMAIN_FILE" >>"$DOMAIN_FILE" && _green 'Added\n'
    fi
